...
{
    // 修正：类名改为FolderOperations，避免与原有类重复
        ...
        public static void CleanBugsLog(string folderPath, string excludeFileName)
        {
            ...
            try
            {
                foreach (var filePath in Directory.GetFiles(folderPath))
                {
                    ...
                    // 可以添加过期判断逻辑（例如：删除7天前的日志）
                    File.Delete(filePath);
                }
                LogManager.GetInstance().AddLog("Bugs日志", "已清理过期日志文件");
                MessageBox.Show("清理完成（保留Game.log）", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                LogManager.GetInstance().AddLog("Bugs日志", $"清理失败：{ex.Message}", "错误");
